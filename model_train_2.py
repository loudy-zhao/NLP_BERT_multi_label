...
# 运行命令 python model_train.py --bert_model_name=./bert_model  --train_data=./data_input/train.csv --lab_flag=lsm_test_001  --model_save_path=./model_dump/lsm_test_001
...
def run_cv(nfold, data, model_save_path, epoch_num):
    ...
    train_model_pred = np.zeros((len(data), NCLASS))
    ...
    for i, (train_fold, test_fold) in enumerate(kf):
        ...
        checkpoint = ModelCheckpoint(model_save_final_str, monitor='val_precision',
                                     verbose=2, save_best_only=False, mode='max', save_weights_only=False,
                                     )
